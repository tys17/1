...
class SensorModel:
    ...
    def compute_Phit(self, z, zprime):
        tempt = np.divide(1,
                          self.generate_normaldis(self.max_range, zprime, self.sigma_hit) - self.generate_normaldis(0,
                                                                                                                    zprime,
                                                                                                                    self.sigma_hit))
        p = np.multily(tempt, 1.0 / math.sqrt(2 * math.pi * self.sigma_hit ** 2) * np.exp(
    ...
    def beam_range_finder_model(self, z_t1_arr, x_t1):
        ...
        # visualization
        draw_laser_beam(z_t1_prime, x_t1)
